...
Os comandos e flags citados aparecem na notação do GCC e do CMake por serem as mais difundidas, como ilustração. O conjunto real adotado por um projeto — compilador, padrão, flags, ferramenta de análise — é fato específico daquele projeto e mora no seu `docs/workflow.md`, não aqui.
...
2. **Warning é erro.** O build roda no mínimo com `-Wall -Wextra`, e `-Werror` é o alvo. Projeto legado que ainda não passa nessa configuração adota o ratchet da Seção 7 — não desliga o aviso.
...
7. **Extensão de compilador é decisão consciente**, registrada em ADR quando afeta portabilidade. Código que depende de extensão fica isolado na camada de hardware.
...
8. **Precedência explícita por parênteses** em expressão composta, mesmo quando a precedência da linguagem já garantiria o resultado.

## 3. Memória e recursos

1. **Sem alocação dinâmica.** Nada de `malloc`/`free` em tempo de execução. Onde a alocação for inevitável, ela acontece uma única vez na inicialização e nunca é liberada — e isso é registrado em ADR.
...
6. **Estruturas usadas em protocolo ou persistência não dependem de layout implícito**: alinhamento, preenchimento e ordem de bytes são tratados explicitamente na serialização, não assumidos do compilador.
...
7. **`volatile` marca o que muda fora do fluxo do programa** — registrador de periférico, variável escrita por interrupção. `volatile` **não** é mecanismo de sincronização: acesso compartilhado precisa de seção crítica ou primitiva do RTOS (ver [firmware.md](firmware.md)).

## 6. Adoção do MISRA

MISRA C é um subconjunto seguro da linguagem, publicado como documento normativo. O texto das regras **não é reproduzido neste guia** — ele é protegido por direito autoral e só é citável a partir do documento oficial. O que este guia define é o processo de adoção.
...
4. **Aplica-se a código novo primeiro.** Código legado entra pelo ratchet da Seção 7 e é migrado quando a área for tocada por outra tarefa — nunca por mutirão de reformatação, que gera diff irrevisável (ver [engenharia.md](engenharia.md), Seção 2).
...
6. **As regras deste arquivo não substituem o documento.** Boa parte do que está nas Seções 2 a 5 coincide com o espírito do MISRA e é adotável hoje, sem o documento e sem ferramenta — mas conformidade declarada exige o documento e a ferramenta.

## 7. Sensores da linguagem e adoção incremental

Sensor é verificação automática posterior; regra é orientação prévia ([engenharia.md](engenharia.md), Seção 3). Este domínio acrescenta ao conjunto mínimo de sensores:
...
- **A linha de base é indexada por arquivo e regra, nunca por número de linha.** Congelar `driver.c:412` faz com que qualquer inserção acima reabra o achado antigo e mascare o novo; em uma semana o sensor vira ruído e alguém o desliga.
...
- **Achado suprimido tem justificativa no próprio local** e, quando for desvio de regra adotada, o registro da Seção 6.
